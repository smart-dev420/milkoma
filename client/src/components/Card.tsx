...
import { fontBold, fontSize12, fontSize18, staticFiles } from "./Constants";
          ...
          {elements.map((el) => (
            <Grid item xs="auto" key={el.name} > 
              <div className={` w-[318px] mx-[28px] bg-cover bg-center justify-between card-aspect rounded-[20px] card-padding`} style={{backgroundImage: `url(${el.imagePath})`}}>
                ...
                <label className="font-m1c font-medium " style={{fontWeight:fontBold, textAlign:'center', fontSize:fontSize18}}>{el.name}</label>
                <p className="mt-1" style={{fontSize:fontSize12}}>{el.description}</p>
